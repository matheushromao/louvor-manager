...
<div class="mx-auto max-w-3xl">
    <div class="rounded-3xl bg-white p-6 shadow-soft ring-1 ring-slate-900/5 sm:p-8 card-panel">
        ...
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-slate-900">Editar Repertório</h1>
            <p class="mt-1 text-sm text-slate-500">Atualize o nome, a data e as músicas deste repertório.</p>
        </div>

        <form action="{{ route('repertorios.update', $repertorio->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <x-input type="text" name="nome" value="{{ old('nome', $repertorio->nome) }}" />
                    <x-error field="nome" />
                </div>

                <div>
                    <x-label>Data</x-label>
                    <x-input type="date" name="data" value="{{ old('data', $repertorio->data) }}" />
                    <x-error field="data" />
                </div>
            </div>
                    ...
                    @forelse($musicas as $musica)
                        <label class="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white">
                            <input
                                ...
                                name="musicas[]"
                                value="{{ $musica->id }}"
                                @checked(collect(old('musicas', $repertorio->musicas->pluck('id')->all()))->contains($musica->id))
