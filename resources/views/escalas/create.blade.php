...
<div class="mx-auto max-w-3xl">
    <div class="rounded-3xl bg-white p-6 shadow-soft ring-1 ring-slate-900/5 sm:p-8 card-panel">
        ...
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-slate-900">Nova Escala</h1>
            <p class="mt-1 text-sm text-slate-500">Defina a data do culto e selecione os vocais escalados.</p>
        </div>

        <form action="{{ route('escalas.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea
                    ...
                    class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 transition placeholder:text-slate-400 focus:border-[var(--site-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--site-primary)]/20">{{ old('observacao') }}</textarea>
                    ...
                    @forelse($vocais as $vocal)
                        <label class="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white">
                            <input
                                ...
                                name="usuarios[]"
                                value="{{ $vocal->id }}"
                                @checked(collect(old('usuarios'))->contains($vocal->id))
